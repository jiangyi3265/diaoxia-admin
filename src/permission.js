...
const xyRoutePermissions = {
  ...
  '/xiayu/finance': 'xy:finance:view',
  '/xiayu/staff': 'xy:staff:list'
}
...
router.beforeEach((to, from, next) => {
  ...
  if (getToken()) {
    ...
    if (to.path === '/login') {
      next({ path: '/' })
      NProgress.done()
    } else if (useUserStore().roles.length > 0 && xyRoutePermissions[to.path] && !auth.hasPermi(xyRoutePermissions[to.path])) {
