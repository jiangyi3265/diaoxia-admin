        ...
        <template v-if="activeEvent">
          ...
          <div class="seat-and-notice">
            <div class="seat-panel">
              ...
              <div class="pool-map" aria-label="福利钓22座座位图">
                <div class="seat-row top-row"><button v-for="n in topSeats" :key="n" type="button" :class="seatClass(n)" @click="focusSeat(n)">{{ n }}</button></div>
                <div class="pool-middle">
                  <div class="side-seats"><button v-for="n in [19,20]" :key="n" type="button" :class="seatClass(n)" @click="focusSeat(n)">{{ n }}</button></div>
                  <div class="pool"><span>33平方小池</span><small>福利钓专场</small></div>
                  <div class="side-seats"><button v-for="n in [21,22]" :key="n" type="button" :class="seatClass(n)" @click="focusSeat(n)">{{ n }}</button></div>
                </div>
                <div class="seat-row bottom-row"><button v-for="n in bottomSeats" :key="n" type="button" :class="seatClass(n)" @click="focusSeat(n)">{{ n }}</button></div>
              </div>
            </div>
