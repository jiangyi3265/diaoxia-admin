        ...
        <template v-if="activeEvent">
          <div class="detail-head">
            ...
            <div class="detail-actions">
              ...
              <el-button v-if="deleteVisible" v-hasPermi="['xy:benefit:edit']" type="danger" :disabled="!canDelete" :title="deleteBlockedReason" :loading="acting" @click="deleteEvent">删除场次</el-button>
            </div>
          </div>
...
import { cancelXyBenefitEvent, confirmXyBenefitEvent, deleteXyBenefitEvent, getXyBenefitEvent, getXyBenefitEvents, getXyNotificationSettings, getXyReservationConfiguration, refundXyBenefitBooking, saveXyBenefitEvent } from '@/api/xy'
...
const deleteVisible = computed(() => activeEvent.value && ['DRAFT','CANCELED'].includes(activeEvent.value.status))
const deleteBlockedReason = computed(() => {
  if (!deleteVisible.value) return ''
  const unfinished = bookings.value.some(row => {
    const unpaidClosed = row.paymentStatus === 'CLOSED' && !row.refundStatus
    const refundCompleted = row.paymentStatus === 'REFUNDED' && row.refundStatus === 'SUCCESS'
    return row.status !== 'CLOSED' || (!unpaidClosed && !refundCompleted)
  })
  return unfinished ? '仍有占座、报名、支付或资金记录未处理完成，暂时不能删除' : ''
})
const canDelete = computed(() => deleteVisible.value && !deleteBlockedReason.value)
...
async function deleteEvent() {
  const eventDate = activeEvent.value?.eventDate || '当前'
  try {
    await ElMessageBox.confirm(
      `确定删除 ${eventDate} 的福利钓场次吗？删除后该场次会从场次列表移除，但相关财务流水、支付记录和操作审计仍会保留。`,
      '删除福利钓场次',
      { type: 'warning', confirmButtonText: '确认删除', cancelButtonText: '暂不删除', confirmButtonClass: 'el-button--danger' }
    )
  } catch { return }
  acting.value = true
  try {
    await deleteXyBenefitEvent(activeId.value)
    activeId.value = undefined
    activeEvent.value = null
    ElMessage.success('场次已删除，相关财务与审计记录仍已保留')
    await load()
  } finally { acting.value = false }
}
...
@media(max-width:1200px){.benefit-stats{grid-template-columns:repeat(2,minmax(0,1fr))}.seat-and-notice{grid-template-columns:1fr}}
...
@media(max-width:560px){.benefit-stats,.event-list,.form-grid,.fixed-strip{grid-template-columns:1fr}.fee-editor{align-items:flex-start;flex-direction:column}.fee-editor .el-input-number{width:100%}.detail-actions{display:grid;grid-template-columns:1fr;width:100%}.detail-actions .el-button{width:100%;margin-left:0}}
</style>
